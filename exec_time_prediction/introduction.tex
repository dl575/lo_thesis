...
%However, these approaches are typically coarse-grained
%\cite{pacora-hotpar11, paragon-asplos13, quasar-asplos14} and/or reactive
%\cite{gu-dac08, pegasus-isca14, octopusman-hpca15, nachiappan-hpca15}.
...
In this paper, we present an automated and general framework for creating
...
flow.
%In this paper, we present an automated and general framework for creating
%prediction-based DVFS controllers. Our method
%uses variations in input and program state values to predict the appropriate DVFS level
%for a task to meet user response-time requirements with minimal energy consumption.  As the nature of this
%input-dependent variation depends on the task semantics, the predictors created
%are task-specific.  However, we present a general and automated methodology for
%constructing these task-specific predictors.
%Our prediction methodology is based on the intuition that the majority of execution time
%variation can be explained by differences in control flow.
...
%From these features, we 
%predict the execution time of the task before it executes. Then, we can use this
%predicted execution time in order to determine an appropriate DVFS level 
%to minimize energy usage and just meet the response-time deadline.
...
The rest of the paper is organized as follows. Section~\ref{sec:apps} discusses
