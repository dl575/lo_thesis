...
\begin{enumerate}
  ...
  %\item \textbf{oracle}: We implement an ``oracle'' controller which uses the
  %recorded execution times one run as the predicted execution times for a run
...
\begin{figure*}
  ...
  \caption{Normalized energy usage and deadline misses as time budget is varied.}
  \label{fig:evaluation.sweeps}
  \end{center}
\end{figure*}
...
Table~\ref{tab:evaluation.benchmarks}). This corresponds to the tightest budget
such that all jobs are able to meet their deadline.
Figure~\ref{fig:evaluation.sweeps} shows the energy usage and deadline misses
for the various benchmarks as the normalized budget is swept below and above 1. 
We see that our prediction-based controller is able to save more energy with
longer time budgets and continues to outperform the interactive governor and
the PID-based controller for varying time budgets.
For normalized budgets less than 1, our prediction-based controller shows
deadline misses. However, the number of misses is typically close to
the number seen with the performance governor.  This implies that the only
deadline misses are the ones that are impossible to meet at the specified time
budget, even with running at the maximum frequency.
...
%\begin{figure*}
%  \begin{center}
%    \subfloat[Average time to run prediction slice and switch DVFS levels.]{
%      \includegraphics[width=\columnwidth]{data/overhead_time.pdf}
%      \label{fig:evaluation.overhead_time}
%    }
%    \hfill
%    \subfloat[Normalized energy usage and deadline misses with overheads removed and oracle prediction.]{
%      \includegraphics[width=\columnwidth]{data/overhead_comparison.pdf}
%      \label{fig:evaluation.overhead_comparison}
%    }
%    \vspace{-0.1in}
%    \caption{Analysis of overheads and prediction error.}
%    \vspace{-0.1in}
%    \label{fig:easdfvaluation.overhead_time}
%  \end{center}
%\end{figure*}
...
% On our prototype platform, DVFS levels are changed by writing to a device file.
% A more direct interface and/or faster DVFS switching circuits
% \cite{booster-hpca12, shortstop-vlsic13, fgsync-micro14} could reduce this
% overhead. Switching times on the order of tens of nanoseconds have been
% reported.
